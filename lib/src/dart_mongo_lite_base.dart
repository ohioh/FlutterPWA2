...
class Database {
  ...
  void _loadFileContent() {
    ...
    try {
      _dbContent = (json.decode(content) as Map<String, dynamic>)
...
class Collection {
  ...
  List<Map<String, dynamic>> find({Map<String, dynamic> filter}) {
    _db._sync();
    return List.from(
        _db._dbContent[_name].where((e) => _applyFilter(e, filter)));
  }

  Map<String, dynamic> findOne({Map<String, dynamic> filter}) {
    return find()
        .firstWhere((e) => _applyFilter(e, filter), orElse: () => null);
  }

  List<T> findAs<T>(T Function(Map<String, dynamic> v) predicate,
      {Map<String, dynamic> filter}) {
    return List.from(find(filter: filter).map(predicate));
  }

  T findOneAs<T>(T Function(Map<String, dynamic> v) predicate,
      {Map<String, dynamic> filter}) {
